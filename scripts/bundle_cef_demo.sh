...
# Bundle the cef-demo binary into a macOS .app with the CEF framework/helpers.
...
<dict>
  ...
  <string>cef-demo</string>
  <key>CFBundleIdentifier</key>
  <string>com.rune.cef-demo</string>
...
copy_helpers() {
  ...
  for suffix in "${variants[@]}"; do
    ...
    local dest="$APP_DIR/Contents/Frameworks/cef-demo Helper${suffix}.app"
    if [[ ! -d "$src" ]]; then
    ...
    local exec_dest="$dest/Contents/MacOS/cef-demo Helper${suffix}"
    cp "$exec_src" "$exec_dest"
    chmod +x "$exec_dest"

    local bundle_id="com.rune.cef-demo.helper"
    case "$suffix" in
    ...
    write_helper_plist "$dest" "cef-demo Helper${suffix}" "cef-demo Helper${suffix}" "$bundle_id"
  done
}
...
copy_framework() {
  ...
  if [[ -d "$lib_src" ]]; then
    for helper in "$APP_DIR"/Contents/Frameworks/cef-demo\ Helper*.app; do
      [[ -d "$helper" ]] || continue
