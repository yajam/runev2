...
pub fn translate_window_event(event: &winit::event::WindowEvent) -> Option<RuneWindowEvent> {
    ...
    match event {
        ...
        WindowEvent::ScaleFactorChanged { scale_factor, .. } => Some(RuneWindowEvent::ScaleFactorChanged(*scale_factor)),
        WindowEvent::CursorMoved { position, .. } => {
            Some(RuneWindowEvent::CursorMoved { position: [position.x as f32, position.y as f32] })
        }
