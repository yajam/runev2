...
impl InputBox {
    pub fn render(&self, canvas: &mut Canvas, z: i32) {
        ...
        // Text (left padding)
        let tp = [self.rect.x + 8.0, self.rect.y + self.rect.h * 0.5 + self.text_size * 0.35];
        canvas.draw_text_run(tp, self.text.clone(), self.text_size, Color::rgba(16, 16, 16, 255), z + 2);

        // Caret if focused (simple left caret)
        if self.focused {
            let cx = tp[0] + 2.0; // not measuring; simple caret
            let cy0 = self.rect.y + 6.0;
            let cy1 = self.rect.y + self.rect.h - 6.0;
            let mut caret = Path { cmds: Vec::new(), fill_rule: FillRule::NonZero };
    ...
    pub fn render_with_provider(
        ...
        let opts = LayoutOptions { size_px: self.text_size, wrap: Wrap::NoWrap, start_baseline_y: 0.0, line_pad: 0.0, scale_factor };
        ...
        if let Some(m) = measure_run(provider, &self.text, self.text_size) {
            ...
            if m.width > max_w {
                ...
                for ch in text.chars() {
                    ...
                    if measure_run(provider, &cand, self.text_size).map(|mm| mm.width).unwrap_or(0.0) <= max_w { acc = cand; } else { break; }
                }
                text = acc;
            }
            canvas.draw_text_run([x, baseline], text, self.text_size, Color::rgba(16, 16, 16, 255), z + 2);
            // Caret
            if self.focused {
                let caret_x = x + measure_run(provider, &self.text, self.text_size).map(|mm| mm.width.min(max_w)).unwrap_or(0.0) + 2.0;
