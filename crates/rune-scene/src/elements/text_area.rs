...
impl TextArea {
    pub fn new(
        ...
        // Note: TextLayout uses font's native line_gap which may be large
        // We render with the layout as-is since modifying line positions
        // would break cursor positioning and hit testing
        ...
        Self {
            rect, text, text_size, text_color, placeholder, focused,
            caret: CaretBlink::new(focused),
            cursor_position: initial_cursor,
            scroll_y: 0.0,
            padding_x, padding_y: 8.0,
            min_height: Some(60.0), max_height: None,
            rt_layout,
            rt_selection: RtSelection::collapsed(initial_cursor),
            mouse_selecting: false, last_mouse_pos: None,
        ...
        if let (Some(layout), Some(wrap_width)) = (self.rt_layout.as_mut(), self.wrap_width) {
            if let Ok(font) = load_system_default_font() {
                *layout = RtTextLayout::with_wrap(
                    self.text.clone(), &font, self.text_size,
                    Some(wrap_width), RtWrapMode::BreakWord,
                );
            }
        }
    }
    ...
    pub fn calculate_content_height(&self) -> f32 {
        ...
        if let Some(min) = self.min_height { height = height.max(min); }
        if let Some(max) = self.max_height { height = height.min(max); }
        height
    }
    
    pub fn update_scroll(&mut self) {
        let layout = match self.rt_layout.as_ref() { Some(l) => l, None => return };
    ...
    where
        F: FnOnce(&mut RtTextLayout, &rune_text::FontFace, &RtSelection, f32) -> (usize, RtSelection),
    {
        self.clamp_selection_to_layout();
        let selection_before = self.rt_selection.clone();
        let size = self.text_size;
        let (new_cursor, new_selection, new_text) = {
            let layout = match self.rt_layout.as_mut() { Some(l) => l, None => return };
            let font = match load_system_default_font() { Ok(f) => f, Err(_) => return };
    ...
    pub fn insert_char(&mut self, ch: char) {
        self.with_layout_edit(|layout, font, selection, size| {
            ...
            let new_cursor = if selection.is_collapsed() {
                layout.insert_char(selection.active().min(layout.text().len()), ch, font, size, Some(wrap_width), RtWrapMode::BreakWord)
            } else {
                layout.replace_selection(selection, &text, font, size, Some(wrap_width), RtWrapMode::BreakWord)
            };
    ...
    pub fn delete_before_cursor(&mut self) {
        if self.text.is_empty() || self.cursor_position == 0 { return; }
        self.with_layout_edit(|layout, font, selection, size| {
            let wrap_width = layout.max_line_width().max(100.0);
            let new_cursor = if selection.is_collapsed() {
                layout.delete_backward(selection.active(), font, size, Some(wrap_width), RtWrapMode::BreakWord)
            } else {
                layout.delete_selection(selection, font, size, Some(wrap_width), RtWrapMode::BreakWord)
            };
            (new_cursor, RtSelection::collapsed(new_cursor))
        });
    }
    ...
    pub fn delete_after_cursor(&mut self) {
        if self.text.is_empty() || self.cursor_position >= self.text.len() { return; }
        self.with_layout_edit(|layout, font, selection, size| {
            let wrap_width = layout.max_line_width().max(100.0);
            let new_cursor = if selection.is_collapsed() {
                layout.delete_forward(selection.active(), font, size, Some(wrap_width), RtWrapMode::BreakWord)
            } else {
                layout.delete_selection(selection, font, size, Some(wrap_width), RtWrapMode::BreakWord)
            };
            (new_cursor, RtSelection::collapsed(new_cursor))
    ...
    pub fn move_cursor_right(&mut self) {
        ...
        if self.cursor_position >= self.text.len() { return; }
        let new_cursor = {
            let layout = match self.rt_layout.as_ref() { Some(l) => l, None => return };
            let mut pos = self.cursor_position.min(layout.text().len());
    ...
    pub fn contains_point(&self, screen_x: f32, screen_y: f32) -> bool {
        screen_x >= self.rect.x && screen_x <= self.rect.x + self.rect.w &&
        screen_y >= self.rect.y && screen_y <= self.rect.y + self.rect.h
    }
    ...
    pub fn cut_to_clipboard(&mut self) -> Result<(), String> {
        if self.rt_selection.is_collapsed() { return Ok(()); }
        let result = {
            ...
            layout.cut_to_clipboard(&selection, &font, size, Some(wrap_width), RtWrapMode::BreakWord)
        };
        match result {
            Ok(new_cursor) => {
                if let Some(layout) = self.rt_layout.as_ref() {
    ...
    pub fn paste_from_clipboard(&mut self) -> Result<(), String> {
        let result = {
            ...
            if selection.is_collapsed() {
                layout.paste_from_clipboard(selection.active(), &font, size, Some(wrap_width), RtWrapMode::BreakWord)
            } else {
                layout.paste_replace_selection(&selection, &font, size, Some(wrap_width), RtWrapMode::BreakWord)
            }
        };
        match result {
            Ok(new_cursor) => {
    ...
    pub fn undo(&mut self) -> bool {
        let result = {
            ...
            let wrap_width = self.wrap_width.unwrap_or(100.0);
            layout.undo(&selection, &font, size, Some(wrap_width), RtWrapMode::BreakWord)
        };
    ...
    pub fn redo(&mut self) -> bool {
        let result = {
            ...
            let wrap_width = self.wrap_width.unwrap_or(100.0);
            layout.redo(&selection, &font, size, Some(wrap_width), RtWrapMode::BreakWord)
        };
        if let Some((new_cursor, new_selection)) = result {
            if let Some(layout) = self.rt_layout.as_ref() {
                self.text = layout.text().to_string();
